...
const text = {
  ru: {
    ...
    landingRuntime: "Готовая ссылка",
    landingDiagnostics: "Видно каждый вызов",
    landingPrivateNote: "Доступ по ролям",
    landingRuntimeNote: "Без локальных скриптов",
    landingDiagnosticsNote: "Источник ответа в логах",
    ...
    landingProblemCopy: "Контракт уже согласован, макеты готовы, задачи взяты в работу — но нормального API ещё нет. В итоге появляются временные JSON-файлы, локальные заглушки и долгие вопросы “а что именно отправлял клиент?”. MSaaS закрывает этот промежуток.",
    landingProblems: [
  ...
  en: {
    ...
    landingRuntime: "Ready-to-call link",
    landingDiagnostics: "Every call is visible",
    landingPrivateNote: "Role-based access",
    landingRuntimeNote: "No local scripts",
    landingDiagnosticsNote: "Response source in logs",
    ...
    landingProblemCopy: "The contract is agreed, screens are ready, tickets are already in progress — but there is still no real API to call. That is when temporary JSON files, local stubs, and long “what did the client send?” conversations appear. MSaaS covers that gap.",
    landingProblems: [
...
export default function ConsoleApp() {
  ...
  "paid": true
          ...
          <section className="landing-section landing-diagnostics" id="landing-diagnostics">
            ...
            <div className="landing-section-heading">
              ...
              <button className="primary-button" onClick={() => openRegisterScreen()}>
                <KeyRound size={17} />
  ...
  return (
    <main className="console-shell">
      ...
      {selectedLog && (
        <div className="modal-backdrop log-modal-backdrop" role="presentation" onClick={() => setSelectedLog(null)}>
        <aside className="drawer log-detail-dialog" role="dialog" aria-modal="true" aria-label="log detail" onClick={(event) => event.stopPropagation()}>
          <div className="drawer-heading">
            <div>
              <p className="eyebrow">{t.logs}</p>
              <h2>{selectedLog.method} {selectedLog.path}</h2>
            </div>
            <button className="icon-button" aria-label={lang === "ru" ? "Закрыть" : "Close"} onClick={() => setSelectedLog(null)}><XCircle size={18} /></button>
          </div>
